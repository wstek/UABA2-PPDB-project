        ...
        for i in range(len(self.abtest["algorithms"])):
            ...
            if self.abtest["algorithms"][i]["name"] == "ItemKNN":
                ...
                "dt_start_RetrainInterval":pd.to_datetime(self.abtest["start"], format='%Y-%m-%d'), "KNN":ItemKNNIterative(k=int( \
                    self.abtest["algorithms"][i]["parameters"]["KNearest"]), normalize=self.abtest["algorithms"][i]["parameters"]["Normalize"])}
            ...
            for algo in range(len(self.abtest["algorithms"])):
                ...
                if self.abtest["algorithms"][algo]["name"] == "ItemKNN":
                    ...
                    if n_day:
                        ...
                        interactions = db_engine.session.execute(f"SELECT DISTINCT customer_id, article_id from purchase WHERE \
                        ...
                        if (retrain >= int(self.abtest["algorithms"][algo]["parameters"]['RetrainInterval'])):
                            ...
                            dynamic_info_algorithms[idx]["dt_start_RetrainInterval"] = dt_current_date - pd.DateOffset(days= \
                                (retrain-int(self.abtest["algorithms"][algo]["parameters"]['RetrainInterval'])))
                        ...
                        for i in range(len(active_users)):
                            data_per_user_over_time_statistics['customer_id'][active_users[i]][-1].algorithm_data.append(RANDOM_DATA(id=idx, topk=top_k_random, name="ItemKNN"))
                    ...
                    if n_day:
                        ...
                        if (retrain >= int(self.abtest["algorithms"][algo]["parameters"]['RetrainInterval'])):
                            ...
                            dynamic_info_algorithms[idx]["dt_start_RetrainInterval"] = dt_current_date - pd.DateOffset(days=(retrain-int(self.abtest["algorithms"][algo]["parameters"]['RetrainInterval'])))
                        else:
                            top_k_items = copy.deepcopy(dynamic_info_algorithms[idx]["prev_top_k"])
                            # top_k_over_time_statistics[idx].append(copy.deepcopy(top_k_over_time_statistics[idx][-1]))
                        
                        print(f"algorithm {algo}: top_k (BETWEEN {start_date} AND {prev_day}):",top_k_items)
                        
                        for i in range(len(active_users)):
                            data_per_user_over_time_statistics['customer_id'][active_users[i]][-1].algorithm_data.append(RECENCY_DATA(id=idx, topk=top_k_items))
                        ...
                        remove_tuples(top_k_random)
                        for i in range(len(active_users)):
                            data_per_user_over_time_statistics['customer_id'][active_users[i]][-1].algorithm_data.append(RANDOM_DATA(id=idx, topk=top_k_random, name="Recency"))
                    ...
                    if n_day:
                        ...
                        if (retrain >= int(self.abtest["algorithms"][algo]["parameters"]['RetrainInterval'])):
                            ...
                            top_k = db_engine.session.execute(f"SELECT SUBQUERY.article_id, count(*) AS popular_items FROM \
                                (SELECT * FROM purchase WHERE CAST(timestamp as DATE) BETWEEN '{start_date}' AND '{prev_day}') AS SUBQUERY GROUP \
                                    BY SUBQUERY.article_id ORDER BY popular_items DESC LIMIT {k}").fetchall() #bij eerste dag hebben we geen data?
                        ...
                        for i in range(len(active_users)):
                            data_per_user_over_time_statistics['customer_id'][active_users[i]][-1].algorithm_data.append(POPULARITY_DATA(id=idx, topk=top_k_items))
                        ...
                        remove_tuples(top_k_random)
                        for i in range(len(active_users)):
                            data_per_user_over_time_statistics['customer_id'][active_users[i]][-1].algorithm_data.append(RANDOM_DATA(id=idx, topk=top_k_random, name="Popularity"))
                        dynamic_info_algorithms[idx]["prev_top_k"] = top_k_random
                        print(f"algorithm {algo}: top_k random:",top_k_random)
                ...
                time.sleep(0.3)
                self.progress = i*100.0/float(dayz)
        return
